...
def main():
    ...
    if training_args.do_eval:
        ...
        # Save original scalings
        original_scalings = {}
        for name, module in model.named_modules():
            if isinstance(module, LoraLayer):
                original_scalings[name] = {k: v for k, v in module.scaling.items()}
                
        def is_rank_0():
            return not dist.is_initialized() or dist.get_rank() == 0
                
        for w in weights:
            logger.info(f"\nEvaluating with LoRA weight multiplier: {w}")
            # adjust weight
            for name, module in model.named_modules():
                if isinstance(module, LoraLayer):
                    for adapter_name in module.scaling.keys():
                        module.scaling[adapter_name] = original_scalings[name][adapter_name] * w

            result_entry = {"weight": w}
            
            # eval_dataset
            if eval_dataset is not None:
                metrics = trainer.evaluate(eval_dataset=eval_dataset, **model.generation_config.to_dict())
                result_entry["eval_metrics"] = metrics
            ...
            try:
                ...
                if mmlu_result:
                    result_entry["mmlu_metrics"] = mmlu_result
            except Exception as e:
                logger.exception(f"MMLU eval failed for weight {w}: {e}")
                
            all_results.append(result_entry)
            
        if is_rank_0():
            weight_results_path = os.path.join(training_args.output_dir, "weight_search_results.json")
            with open(weight_results_path, "w", encoding="utf-8") as f:
                json.dump(all_results, f, ensure_ascii=False, indent=2)
            logger.info(f"All weight search results saved to {weight_results_path}")
            
            # restore original weights just in case
            for name, module in model.named_modules():
                if isinstance(module, LoraLayer):
                    for adapter_name in module.scaling.keys():
                        module.scaling[adapter_name] = original_scalings[name][adapter_name]
