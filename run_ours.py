...
from roupct_trainer import RoupCTTrainer
...
class MyTrainingArguments(Seq2SeqTrainingArguments):
    ...
    mmlu_ntrain: Optional[int] = field(
        default=5)  # Few-shot k for MMLU: prepend k dev examples before each test question.
    ...
    # RoupCT-specific hyperparameters for grid search / shell scripts.
    roupct_alpha: Optional[float] = field(default=0.05)
    roupct_lambda_pen: Optional[float] = field(default=0.1)
    roupct_seq_len: Optional[int] = field(default=16)
    roupct_tau: Optional[float] = field(default=0.5)
    roupct_soft_prompt_lr: Optional[float] = field(default=1e-3)
...
def main():
    ...
    for task_i, (task_train_dataset, task_name) in enumerate(zip(train_datasets, train_task_list)):
        ...
        if training_args.do_train:
            ...
            # Post-task gradient accumulation for memory
            lora_params = trainer._get_lora_params()
            if not hasattr(trainer, 'initial_lora_weights'):
                trainer.initial_lora_weights = [p.data.clone() for p in lora_params]

            # g_i = initial - current
            task_grad = trainer._flatten_grads([orig - p.data for orig, p in zip(trainer.initial_lora_weights, lora_params)])
            trainer.M_old.append(task_grad.detach().float().clone())
            
            # reset initial for next task
            trainer.initial_lora_weights = [p.data.clone() for p in lora_params]
